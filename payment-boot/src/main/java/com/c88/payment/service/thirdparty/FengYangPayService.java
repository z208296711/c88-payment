...
public class FengYangPayService implements IThirdPartPayService {

    private final RestTemplate restTemplate;

    private static final String MERCHANT_CODE = "fp";
    ...
    public Result<ThirdPartyPaymentVO> createPayOrder(Merchant merchant, BasePayDTO payDTO) {
        ...
        if (SUCCESS != status) {
            return Result.failed(ResultCode.INTERNAL_SERVICE_CALLEE_ERROR, "fengyang pay error code: " + status);
        }
    ...
    private Map<String, Object> generateBehalfPayReq(BaseBehalfPayDTO behalfPayDTO, Merchant merchant) {
        ...
        req.put("bank_province", behalfPayDTO.getBankProvince());
        req.put("bank_sub", behalfPayDTO.getBankSub());
        req.put("bank_city", behalfPayDTO.getBankCity());
        req.put("sign", this.encrypt(req, key));
        return req;
    }
