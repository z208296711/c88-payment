...
public class TRPayCallBackImpl implements ICallBackService {
    ...
    public String callBackPay(CallBackGeneralForm form) {
        ...
        MemberWithdraw memberWithdraw = queryWithdraw.apply(iWithdrawService, form.getOrder_id())
                .orElseThrow(() -> new BizException("trPay 無此代付訂單"));

        // 代付訂單成功
        if (Objects.equals(form.getStatus(), ORDER_SUCCESS)) {
            log.info("trPay代付成功 success before {}", form);

            withDrawHandler(iWithdrawService, memberWithdraw.getWithdrawNo(), true);

            log.info("trPay代付成功 success after {}", form);

            return SUCCESS_RESULT;
        }

        // 代付訂單失敗
        log.info("trPay代付失敗 fail before {}", form);
        ...
        log.info("trPay代付失敗 fail after {}", form);
